...
async def atualizar_placar(sid, dados_da_partida):
    global estado_do_torneio

    print(f"Placar atualizado: {dados_da_partida}")

    novo_estado = dados_da_partida
    if isinstance(dados_da_partida, dict):
        novo_estado = dados_da_partida.get("novoEstadoDoTorneio", dados_da_partida)

    if isinstance(novo_estado, list):
        estado_do_torneio = novo_estado
    else:
        estado_do_torneio = dados_da_partida
